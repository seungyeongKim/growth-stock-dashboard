...
async function fetchDartRecentDisclosures(target) {
  if (!dartKey) {
    return { enabled: false, items: [], note: "DART_API_KEY가 없어 공시 자동 수집은 건너뜀" };
  }
  ...
  try {
    ...
    return { enabled: true, items: [], note: `DART 수집 실패: ${error.message}` };
  }
}
...
async function fetchNaverQuote(target) {
  ...
  try {
    ...
    return { enabled: false, source: "Naver Finance realtime", note: `시세 수집 실패: ${error.message}` };
  }
}
